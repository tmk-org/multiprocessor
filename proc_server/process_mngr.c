...
static int process_info_init(struct process_info *info){
    ...
    if (sem_init(&info->sem_job, 1, 0) == -1){
	    fprintf(stderr, "Error: can't init shared process semaphore\n");
	    return -1;
    }
...
int process_start(char *argv[]){
    ...
    if (process_cnt >= process_cnt_max){
	    ...
	    fprintf(stderr, "Error: no more process allowed\n");
	    return -1;
    }
    id = process_cnt++;
    pthread_mutex_unlock(&process_mutex);
    ...
    if ((pid = fork()) == -1){
	    fprintf(stderr, "ERROR: starting new child failed on fork(): errno=%d, %s\n",
    ...
    if (pid == 0){
	    ...
	    if (setenv("SHMEM_PROCESS_ID", id_str, 1) != 0){
	        fprintf(stderr, "ERROR: can't set SHMEM_PROCESS_ID value for child process: errno=%d, %s\n",
		        errno, strerror(errno));
	        exit(EXIT_FAILURE);
	    }
    #if 0
    ...
    #endif
	    execv(argv[0], argv);
	    fprintf(stderr, "ERROR: can't exec child process: errno=%d, %s\n",
		    errno, strerror(errno));
	    exit(EXIT_FAILURE);
    }
...
int process_call(struct process_info *info){
    sem_post(&info->sem_job);
    sem_wait(&info->sem_result);
    return info->cmd_result;
}
