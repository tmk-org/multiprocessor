...
void *finalization_thread(void *arg){
    ...
    while(1){
        ...
        fprintf(stdout, "[finalization_thread]: get data %p, data: %s\n", data, data);
#if 0 //no child process, only thread
        ...
        if (module->proc_info->cmd_result != 0){
            ...
            printf("finalization_thread: destination error %d, drop data %p\n", module->proc_info->cmd_result, data);
            //pushfree buffer
            ring_buffer_push(module->data_ring, dataoffset);
            continue;
        }
        printf("finalization_thread: passed data %p, data: %s\n", data, data);
#endif
        ring_buffer_push(module->data_ring, dataoffset);
    }
    fprintf(stdout, "[finalization_thread]: stop\n");
#if 0 //no child process, only thread
...
void *source_thread(void *arg){
    ...
    while(!(*(module->exit_flag))){
        ...
        fprintf(stdout, "[first_thread]: new data %p, data %s\n", data, data);
        module->proc_info->cmd = cmd; 
        module->proc_info->cmd_offs = dataoffset;
        sem_post(&module->proc_info->sem_job);
        sem_wait(&module->proc_info->sem_result);
    ...
    fprintf(stdout, "[first_thread]: stop\n");
    sem_wait(&module->next->result);
...
int model_read_configuration_(char *fname, struct module **p_modules) {
    ...
    for (i = 0; i < modules_count; i++) {
        ...
        if (descriptions[i].type == MT_FIRST) {
            ...
            modules[idx].thread_method = finalization_thread;
        }
        else if (descriptions[i].type == MT_MIDDLE){ //MT_MIDDLE
            modules[idx].thread_method = process_thread;
        }
...
int model_default_configuration(struct module **p_modules) {
    ...
    modules[2].argv[0] = strdup("pmiddle");
    modules[2].argv[1] = NULL;
    modules[2].thread_method = process_thread;
...
void* pipeline_thread(void *arg) {
    struct pipeline *pdesc = (struct pipeline*)(arg);
    void *shmem = pdesc->shmem;
    int i;
    ...
    int modules_count = pdesc->modules_count;
    for (i = 0; i < modules_count; i++) {
        struct module *m = &(pdesc->modules[i]);
        m->shmem = shmem;
        sem_init(&(m->new_cmd), 0, 0);
        sem_init(&(m->result), 0, 1);
        m->cmd = PROCESS_CMD_JOB;
        m->data_ring = data_ring;
        m->exit_flag = &pdesc->exit_flag;
    }

    //start modules threads
    for (i = 0;  i < modules_count; i++) {
        pthread_create(&((pdesc->modules[i]).thread_id), NULL, (pdesc->modules[i]).thread_method, &(pdesc->modules[i])); 
    }
...
struct pipeline *pipeline_init(char *init_string) {
    struct pipeline *pdesc = malloc(sizeof(struct pipeline));
    if (!pdesc) {
        fprintf(stderr, "[pipeline_init]: can't allocte pipeline structure\n");
        fflush(stderr);
        return NULL;
    }
    strcpy(pdesc->shmpath, "/pipeline");
    pdesc->exit_flag = 0;
    void *map = shared_memory_map_create(3, PROCESS_INFO_SIZE, 512, 1024);
    shared_memory_unlink(pdesc->shmpath);
    void *shmem = shared_memory_server_init(pdesc->shmpath, map);
    free(map);

    if (shmem) {
        pdesc->shmem = shmem;
        shared_memory_setenv(pdesc->shmpath, shmem);
        if (init_string) {
            pdesc->modules_count = model_read_configuration_(init_string, &(pdesc->modules));
        }
        else {
            pdesc->modules_count = model_default_configuration(&(pdesc->modules));
        }
        pthread_create(&pdesc->thread_id, NULL, pipeline_thread, pdesc);
    }
    return pdesc;
}
...
void pipeline_destroy(struct pipeline *pdesc) {
    pdesc->exit_flag = 1;
    pthread_join(pdesc->thread_id, NULL);
    shared_memory_destroy(pdesc->shmpath, pdesc->shmem);
    free(pdesc);
}
