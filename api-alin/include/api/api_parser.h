...
char *executeAPICommand(const char *request, int *exit_flag) {
    ...
    if (strncmp(request + parts[3].rm_so, #name, strlen(#name)) == 0) {           \
        currCmd = API_##NAME;                                                     \
        if (type == __API_SWITCH_TYPE_UNKNOWN_OK) {                               \
            if (clientId > 0) {                                                   \
        ...
        } else if (type == __API_SWITCH_TYPE_UNKNOWN_UNKNOWN) {                   \
            asprintf(&reply, "ER017{%d,,wrong command}", currCmd);                \
            return reply;                                                         \
        } else if (type == __API_SWITCH_TYPE_OK_UNKNOWN) {                        \
            if (clientId > 0) {                                                   \
                asprintf(&cmdStr, "scripts/API_"#NAME);                           \
                api##Name(cmdStr, exit_flag);                                     \
            }                                                                     \
        ...
        } else if (type == __API_SWITCH_TYPE_OK_OK) {                             \
            asprintf(&cmdStr, "scripts/API_"#NAME);                               \
            api##Name(cmdStr, exit_flag);                                         \
        }                                                                         \
    } else
