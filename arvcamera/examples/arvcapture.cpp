...
static void new_buffer_cb (ArvStream *stream, ApplicationData *data) {
    ...
    if (buffer != NULL) {
        if (arv_buffer_get_status (buffer) == ARV_BUFFER_STATUS_SUCCESS) {
            ...
            cv::imwrite(name, img);
            fprintf(stdout, "Frame %s is saved\n", name);
...
int main (int argc, char **argv) {
	...
	camera = arv_camera_new(argv[1], &error);
	...
	if (ARV_IS_CAMERA (camera)) {
		...
		//arv_camera_set_region (camera, 0, 0, 640, 480, NULL);
		arv_camera_set_region (camera, 0, 0, 2448, 2048, NULL);
